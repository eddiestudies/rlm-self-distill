...
1. **Claude Opus 4.5 can generate working rules that save tokens.** Experiments 003 and 004 used rules created by Claude Code (Opus 4.5) and achieved 34-43% token savings. The rules ran as pre-completion hooks, avoiding PII cases and handling grammar checks before the main LLM was called.

2. **Smaller models cannot generate usable rules.** When asking smaller models (llama, qwen, deepseek) to create rules, they fail to produce working code. The model capability gap is significant.
...
- **Model for rule creation**: Claude Code (Opus 4.5)
- **Goal**: Use executable Python tools as pre-completion hooks
- **Approach**: Rules generated by Opus 4.5, saved to disk, executed via `importlib`
...
- **Approach**: Safety Macro (generated by Opus 4.5) runs FIRST, masks PII before any LLM call
...
- **Key insight**: Opus 4.5 can generate working rules; smaller models cannot
...
**Opus 4.5-generated rules (SUCCESS):**
...
Rules created by Claude Code (Opus 4.5), not hand-written.
...
1. **Opus 4.5-generated pre-completion rules**: 34-43% token savings (exp003, exp004)
